...
import useLandfillEntryRegist from "@/hooks/useLandfillEntryRegist";
...
const LandfillEntryRegist = () => {
  const [open, onOpenChange] = useState(false);
  const { user } = useUserStore();
    const LandfillEntryRegist = useLandfillEntryRegist(user.id??"",onOpenChange);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        <Button>
          <PlusIcon className="mr-2" /> Add Landfill Entry
        </Button>
      </DialogTrigger>
      <DialogContent
        onInteractOutside={(e) => {
      ...
      >
        <DialogHeader>
          <DialogTitle>Add Landfill Entry</DialogTitle>
          <DialogDescription>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <DialogFooter className="gap-2 pt-10 sm:space-x-0">
                ...
                <Button loading={LandfillEntryRegist.isPending} disabled={!form.formState.isDirty} >Register</Button>
              </DialogFooter>
            </form>
          </Form>
        </div>
...
export default LandfillEntryRegist;
